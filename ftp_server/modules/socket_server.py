...
class Myserver(socketserver.BaseRequestHandler):
    ...
    def get(self,command):
        ...
        if len(command.split()) > 1:
            ...
            if os.path.isfile(file_path):               #文件是否存在
                ...
                status_code = self.conn.recv(1024).decode()    #接收客户端403状态码

                # 客户端存在此文件
                if status_code == "403":
                    self.conn.sendall("000".encode())   #系统交互
                    has_send_size = self.conn.recv(1024).decode()
                    has_send_size = int(has_send_size)   #接收已发送文件的大小
                    # 客户端文件不完整可续传
                    if has_send_size < file_size:
                        self.conn.sendall("205".encode())   #客户端可续传状态码
                        file_size -= has_send_size  #续传文件大小
                        response = self.conn.recv(1024)  # 可续传等待响应
                ...
                response = self.conn.recv(1024)  # 等待响应确认文件大小参数客户端
                with open(file_path,"rb") as file:
                    file.seek(has_send_size)
                    m = hashlib.md5()
                    for line in file:
                        m.update(line)
                        self.conn.sendall(line)               #发送文件内容
    ...
    def mkdir(self,command):
        ...
        if len(command.split()) > 1:
            ...
            dir_path = self.current_path + "\%s"%dir_name #目录路径
    ...
    def __getdirsize(self,home_path):
        ...
        for root, dirs, files in os.walk(home_path):
            size += sum([getsize(join(root, name)) for name in files])     #getisize方法为获取文件大小方法，  使用了列表推导式方法
        return size